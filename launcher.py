...
class AstutePDF:
    ...
    def rotate_box(self):
        ...
        rbutton = Button(popup,text='Done',command = partial(self.rotate,number1,popup,CheckVar1) ,width=50,bg='black',fg='blue')
        rbutton.grid(row=7,column=0,padx=10,pady=10)

    def rotate(self,n1,pp,dir):
        app = QApplication(sys.argv)
        pdf = PDF()
        pp.destroy()
        if dir==1:
    ...
    def summarise_setup(self):
        ...
        rbutton = Button(popup,text='Done',command = partial(self.summarise,popup,CheckVar1) ,width=50,bg='black',fg='blue')
        rbutton.grid(row=7,column=0,padx=10,pady=10)

    def summarise(self,pp,CheckVar1):
        app = QApplication(sys.argv)
        pdf = PDF()
        summ = Summarize()
...
mbutton = Button(root,text='Merge',command = ap.merge,width=50,bg='white',fg='blue')
mbutton.grid(row=2,column=0,padx=10,pady=10)

sbutton = Button(root,text='Split',command = ap.split,width=50,bg='white',fg='blue')
sbutton.grid(row=3,column=0,padx=10,pady=10)

rbutton = Button(root,text='Rotate',command = ap.rotate_box,width=50,bg='white',fg='blue')
rbutton.grid(row=6,column=0,padx=10,pady=10)

obutton = Button(root,text='Summarise',command = ap.summarise_setup,width=50,bg='white',fg='blue')
obutton.grid(row=7,column=0,padx=10,pady=10)

dbutton = Button(root,text='Convert Docx to PDF',command = ap.convert,width=50,bg='white',fg='blue')
